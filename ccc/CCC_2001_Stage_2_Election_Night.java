...
public class CCC_2001_Stage_2_Election_Night {
	...
	public static void main (String[] args) throws IOException {
		...
		// for(int y = 0; y < states+candidates+1; y++)
		// System.out.println(y + " " + adj.get(y));
		for (int x = 1; x <= candidates; x++) {
			...
			if (!willWin[x] && possible[x] >= 1) {
				...
				for (int y = states + 1; y <= states + candidates; y++) {
					...
					curr.get(y).set(index,
							new Edge(states + candidates + 1, possible[x] - 1));
				}
		...
		for (int x = 1; x <= candidates; x++) {
			System.out.println(willWin[x] + " " + mightWin[x]);
		}
		// for(int y = 0; y < states+candidates+1; y++)
		// System.out.println(y + " " + adj.get(y));
	}
	...
	private static int bfs () {
		...
		System.out.println("THE NECK IS " + neck);
		...
		while (prev[c] != -1) {
			...
			System.out.println("CHANGE " + c + " : " + prev[c]);
			c = prev[c];
		}
		return neck;
	}
