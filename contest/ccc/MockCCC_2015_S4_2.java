...
public class MockCCC_2015_S4_2 {
	...
	public static void main (String[] args) throws IOException {
		...
		for (int x = 2; x <= n; x++) {
			left[x] = leftB[x - 1]
					- Math.min(Math.max(0, p[x - 1].d - p[x - 2].w), p[x].w)
					+ p[x].d;
			left[x] = Math.min(left[x], left[x - 1] + p[x].d);
			leftB[x] = Math.min(left[x - 1], leftB[x - 1])
					+ Math.max(0, p[x].d - p[x - 1].w);
		...
		for (int x = n - 1; x >= 1; x--) {
			...
			rightB[x] = Math.min(right[x + 1], rightB[x + 1])
					+ Math.max(0, p[x].d - p[x + 1].w);
		}
		...
		for (int x = 1; x <= n; x++) {
			min = Math.min(
					...
					leftB[x]
							+ right[x + 1]
							- Math.min(Math.max(0, p[x].d - p[x - 1].w),
									p[x + 1].w));
			min = Math.min(
					min,
					left[x - 1]
