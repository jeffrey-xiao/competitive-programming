...
public class Woburn_Challenge_There_Is_No_try {
	...
	public static void main (String[] args) {
		...
		while (x1 != -2) {
				...
				for (int x = -maxJump; x <= maxJump; x++) {
					...
					for (int y = -maxJump; y <= maxJump; y++) {
						if (Math.sqrt(x * x + y * y) > maxJump)
							continue;
						if (p[0] + x >= 0 && p[0] + x < grid[0].length
								&& p[1] + y >= 0 && p[1] + y < grid.length
								&& visited[p[1] + y][p[0] + x] == 5000005
								&& grid[p[1] + y][p[0] + x] == '*') {
			...
			System.out.println(visited[y2][x2] == 5000005 ? "THERE IS NO TRY"
					: visited[y2][x2]);
			y1 = scan.nextInt() - 1;
			x1 = scan.nextInt() - 1;
		}
