	...
	public static void printFriendsOfFriends (String person, ArrayList<String> hasAlready) {
		...
		for (int x = 0; x < friends.get(person).size(); x++) {
			for (int y = 0; y < friends.get(friends.get(person).get(x)).size(); y++) {
				if (!friends.get(person).contains(friends.get(friends.get(person).get(x)).get(y)) && !person.equals(friends.get(friends.get(person).get(x)).get(y)) && !hasAlready.contains(friends.get(friends.get(person).get(x)).get(y))) {
					count++;
					hasAlready.add(friends.get(friends.get(person).get(x)).get(y));
				}
			}
		}
	...
	public static void printSeperation (String person, String target, ArrayList<String> visited, int degree) {
		...
		for (int x = 0; x < friends.get(person).size(); x++) {
			...
			if (!visited.contains(friends.get(person).get(x))) {
				...
				temp.addAll(visited);
				printSeperation(friends.get(person).get(x), target, temp, degree);
			}
		}
	}
}
