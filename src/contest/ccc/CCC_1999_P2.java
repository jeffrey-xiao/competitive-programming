...
public class CCC_1999_P2 {
	...
	public static void main (String[] args) throws IOException {
		...
		for (int x = 0; x < n; x++) {
			...
			int index = 0;
			while (s.indexOf("/", index) != -1 && s.indexOf(".", index) != -1 && s.indexOf(",", index) != -1) {
				if (s.indexOf("/", index) != -1) {
					index = s.indexOf("/", index);
					System.out.println(index);
					if (index >= 3 && index <= s.length() - 6 && s.charAt(index - 1) == ' ' && s.charAt(index + 5) == ' ') {
						if (s.charAt(index + 3) == '/') {
							...
							if (days[Integer.parseInt(month) - 1] >= Integer.parseInt(day))
								s.insert(index + 4, Integer.parseInt(year) <= 24 ? "20" : "19");
						}
					...
					if (index >= 3 && index <= s.length() - 6 && s.charAt(index - 1) == ' ' && s.charAt(index + 5) == ' ') {
						if (s.charAt(index + 3) == '.') {
							...
							if (days[Integer.parseInt(month) - 1] >= Integer.parseInt(day))
								s.insert(index - 1, Integer.parseInt(year) <= 24 ? "20" : "19");
						}
					}
					index += 8;
					System.out.println(index);
