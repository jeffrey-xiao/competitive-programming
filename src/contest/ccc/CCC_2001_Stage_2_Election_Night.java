...
public class CCC_2001_Stage_2_Election_Night {
    ...
    while (states != 0) {
      ...
      for (int x = 1; x <= states; x++) {
        ...
        if (n == 1) {
          ...
          adj.get(x).add(new Edge(states + candidate, Integer.MAX_VALUE));
    ...
    main:
      ...
      for (int x = 1; x <= candidates; x++) {
        if (!willWin[x] && possible[x] >= 1) {
          ...
          for (int y = 1; y <= states; y++) {
            int index = curr.get(y).indexOf(new Edge(states + x, 0));
            if (index != -1) {
          ...
          for (int y = states + 1; y <= states + candidates; y++) {
            ...
            int index = curr.get(y).indexOf(new Edge(states + candidates + 1, 0));
            curr.get(y).set(index, new Edge(states + candidates + 1, possible[x] - 1));
          }
          // REMOVE THE EDGE FROM THE CANDIDATE TO THE SINK
          int index = curr.get(x + states).indexOf(new Edge(states + candidates + 1, 0));
      ...
      for (int x = 1; x <= candidates; x++) {
        if (willWin[x] && possible[x] > 0)
          System.out.println("Candidate " + x + " will become president.");
        else if (mightWin[x] && possible[x] > 0)
          System.out.println("Candidate " + x + " may become president.");
        else
          System.out.println("Candidate " + x + " will not become president.");
      }
      states = readInt();
      candidates = readInt();
    }
  ...
  private static int bfs() {
    ...
    while (!moves.isEmpty()) {
      ...
      for (int x = 0; x < curr.get(currState.index).size(); x++) {
        ...
        if (Math.min(currState.max, next.cost) >= max[next.dest] || next.cost == 0)
          continue;
