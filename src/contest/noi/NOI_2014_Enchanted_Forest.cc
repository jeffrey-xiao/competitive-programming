  ...
  if (uniA.size() < 5000) {
    ...
    while (!q.empty()) {
      ...
      for (pair<int, pi> next : adj[curr]) {
        ...
        if (max(a.first, next.second.first) < na.first ||
            (max(a.first, next.second.first) == na.first &&
                max(a.second, next.second.second) < na.second)) {
          minA[next.first] = mp(max(a.first, next.second.first), max(a.second, next.second.second));
          q.push(next.first);
        }
        if (max(b.second, next.second.second) < nb.second ||
            (max(b.second, next.second.second) == nb.second &&
                max(b.first, next.second.first) < nb.first)) {
          minB[next.first] = mp(max(b.first, next.second.first), max(b.second, next.second.second));
          q.push(next.first);
        }
        if (max(t.second, next.second.second) + max(t.first, next.second.first) <
            nt.first + nt.second) {
    ...
    int res =
        min(min(minA[n - 1].first + minA[n - 1].second, minB[n - 1].first + minB[n - 1].second),
            minT[n - 1].first + minT[n - 1].second);
    if (res > 100000)
      printf("%d\n", -1);
    else
      printf("%d\n", res);
