...
void pushUp(int n){
    ...
    seg_tree[n].segCount = seg_tree[l(n)].segCount + seg_tree[r(n)].segCount - (seg_tree[l(n)].rightColor == seg_tree[r(n)].leftColor);
}
...
int countColor(int l, int r, int num){
    ...
    if(r <= mid)
        ...
        return countColor(l, mid, 2*num) + countColor(mid+1, r, 2*num+1) - (seg_tree[l(num)].rightColor == seg_tree[r(num)].leftColor);
