...
int main () {
    ...
    for (int i = 1; i <= N; i++) {
        ...
        if (dx == 0) {
            ...
            y = py + m * (X1 - px);
            if (between(y, Y1, Y2)) events.push_back(dist(X1 - px, y - py) - d);
            y = py + m * (X2 - px);
            if (between(y, Y1, Y2)) events.push_back(dist(X2 - px, y - py) - d);
            if (dy != 0) {
                x = (m * px + Y1 - py) / m;
                if (between(x, X1, X2)) events.push_back(dist(x - px, Y1 - py) - d);
                x = (m * px + Y2 - py) / m;
                if (between(x, X1, X2)) events.push_back(dist(x - px, Y2 - py) - d);
            }
            if (dx < 0) {
                for (int j = 0; j < (int)events.size(); j++)
                    events[j] = -events[j];
