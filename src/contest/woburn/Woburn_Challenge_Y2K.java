...
public class Woburn_Challenge_Y2K {
	...
	public static void main (String[] args) {
		...
		for (int x = p.size() - 1; x >= 1; x--) {
			if ((p.get(x).getY() - startPoint.getY())
					/ (p.get(x).getX() - startPoint.getX()) == (p.get(x - 1)
					.getY() - startPoint.getY())
					/ (p.get(x - 1).getX() - startPoint.getX())) {
				if (Math.abs(p.get(x).getX() - startPoint.getX()) > Math.abs(p
						.get(x - 1).getX() - startPoint.getX()))
					p.remove(x - 1);
				else
					p.remove(x);
			}
		...
		for (int x = convexHull.size() - 1; x >= 0; x--) {
			System.out.println((int) convexHull.get(x).getX() + " "
					+ (int) convexHull.get(x).getY());
		}
	}
	...
	private static double polar (Point p1, Point p2, Point startPoint) {
		return p1.getX() * p2.getY() + p2.getX() * startPoint.getY()
				+ startPoint.getX() * p1.getY() - p2.getY() * startPoint.getX()
				- startPoint.getY() * p1.getX() - p1.getY() * p2.getX() > 0 ? -1
				: 1;
	}
}
/*
