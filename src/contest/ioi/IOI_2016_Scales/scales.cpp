            ...
            for (int j = 0; j < 4; j++) { // val[j] is on D
                ...
                for (int k = 0; k < 4; k++) { // val[k] is result
                    ...
                    for (int permutationIndex : valid) {
                        ...
                        // D is the heaviest
                        if (toIndex[val[j]] > toIndex[val[(j + 1) % 4]] && toIndex[val[j]] > toIndex[val[(j + 2) % 4]] && toIndex[val[j]] > toIndex[val[(j + 3) % 4]]) {
                            if (toIndex[val[k]] <= toIndex[val[(j + 1) % 4]] && toIndex[val[k]] <= toIndex[val[(j + 2) % 4]] && toIndex[val[k]] <= toIndex[val[(j + 3) % 4]])
                                nextValid.push_back(permutationIndex);
                        }
    ...
    for (int q = 0; q < 6; q++) {
        ...
        if (best.type == 0) {
            ...
            for (int permutationIndex : validSolutions) {
                ...
                if (toIndex[best.val[index]] < toIndex[best.val[(index + 1) % 3]] && toIndex[best.val[index]] < toIndex[best.val[(index + 2) % 3]])
            ...
            for (int permutationIndex : validSolutions) {
                ...
                if (!(((toIndex[best.val[index]] > toIndex[best.val[(index + 1) % 3]] && toIndex[best.val[index]] > toIndex[best.val[(index + 2) % 3]])) ||
                    ((toIndex[best.val[index]] < toIndex[best.val[(index + 1) % 3]] && toIndex[best.val[index]] < toIndex[best.val[(index + 2) % 3]]))))
                    nextSolutions.push_back(permutationIndex);
            }
        } else if (best.type == 2) {
            ...
            int res = getNextLightest(best.val[0] + 1, best.val[1] + 1, best.val[2] + 1, best.val[3] + 1) - 1;
            int index = 0;
            if (res == best.val[1])
                index = 1;
            else if (res == best.val[2])
            ...
            for (int permutationIndex : validSolutions) {
                ...
                int j = 3;
                // D is the heaviest
                if (toIndex[best.val[j]] > toIndex[best.val[(j + 1) % 4]] && toIndex[best.val[j]] > toIndex[best.val[(j + 2) % 4]] && toIndex[best.val[j]] > toIndex[best.val[(j + 3) % 4]]) {
                    if (toIndex[best.val[index]] <= toIndex[best.val[(j + 1) % 4]] && toIndex[best.val[index]] <= toIndex[best.val[(j + 2) % 4]] && toIndex[best.val[index]] <= toIndex[best.val[(j + 3) % 4]])
                        nextSolutions.push_back(permutationIndex);
                }
