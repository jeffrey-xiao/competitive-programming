...
public class DMOPC_2014_Not_Enough_Time {
	...
	public static void main (String[] args) throws IOException {
		...
		dp[0][0] = 0;
		for (int x = 0; x < n; x++)
			p[x] = new Problem(readInt(), readInt(), readInt(), readInt(), readInt(), readInt());
		for (int x = 0; x < n; x++) {
			for (int y = t; y >= 0; y--) {
				if (dp[x % 2][y] >= 0) {
					for (int z = 0; z < 3; z++) {
						...
						if (nextT <= t) {
							dp[(x + 1) % 2][nextT] = Math.max(dp[(x + 1) % 2][nextT], nextP);
						}
					}
				}
				dp[x % 2][y] = Math.max(x - 1 < 0 ? 0 : dp[(x - 1) % 2][y], y - 1 < 0 ? 0 : dp[x % 2][y - 1]);
			}
