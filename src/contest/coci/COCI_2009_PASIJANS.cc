...
#define getchar() (*_pinp?*_pinp++:(_inp[fread(_pinp=_inp, 1, 16777216, stdin)]='\0', *_pinp++))
#define rint(x) do{while((x=getchar())<'0'); for(x-='0'; '0'<=(_=getchar()); x=(x<<3)+(x<<1)+_-'0');}while(0)
...
void pushUp (int n) {
    if (minValue[n << 1] < minValue[n << 1 | 1]) {
        ...
        while (lo <= hi) {
            int mid = (hi + lo) / 2;
            if (getHash(leftIndex, currIndex[leftIndex], currIndex[leftIndex] + mid) == getHash(rightIndex, currIndex[rightIndex], currIndex[rightIndex] + mid))
                lo = mid + 1;
            else
                hi = mid - 1;
        }
