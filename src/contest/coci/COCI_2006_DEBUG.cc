...
int main() {
  ...
  for (int i = 0; i < R; i++) {
    for (int j = 0; j < C; j++) {
      ...
      while (i - currSize >= SIZE && j - currSize >= SIZE && i + currSize + SIZE < R &&
             j + currSize + SIZE < C && canExpandBig1(i, j, currSize))
        currSize += SIZE;

      while (i - currSize >= 1 && j - currSize >= 1 && i + currSize + 1 < R &&
             j + currSize + 1 < C && canExpandSmall1(i, j, currSize))
        currSize++;
  ...
  for (int i = 0; i < R - 1; i++) {
    for (int j = 0; j < C - 1; j++) {
      ...
      while (i - currSize >= SIZE && j - currSize >= SIZE && i + currSize + SIZE + 1 < R &&
             j + currSize + SIZE + 1 < C && canExpandBig2(i, j, currSize))
        currSize += SIZE;

      while (i - currSize >= 1 && j - currSize >= 1 && i + currSize + 2 < R &&
             j + currSize + 2 < C && canExpandSmall2(i, j, currSize))
        currSize++;
