...
public class COCI_2008_DOSTAVA {
	...
	public static void main (String[] args) throws IOException {
		...
		for (int i = 0; i < D; i++) {
			...
			// left; left
			best = Math.min(best, sum[currR][currC - 1] + sum[newR][newC] - sum[newR][1] 
					+ dist[(currR - 1) * 2][(newR - 1) * 2]);
					
			// left; right
			best = Math.min(best, sum[currR][currC - 1] + sum[newR][C - 1] - sum[newR][newC - 1] 
					+ dist[(currR - 1) * 2][(newR - 1) * 2 + 1]);
			
			// right; left
			best = Math.min(best, sum[currR][C] - sum[currR][currC] + sum[newR][newC] - sum[newR][1]
					+ dist[(currR - 1) * 2 + 1][(newR - 1) * 2]);
			
			// right; right;
			best = Math.min(best, sum[currR][C] - sum[currR][currC] + sum[newR][C - 1] - sum[newR][newC - 1]
					+ dist[(currR - 1) * 2 + 1][(newR - 1) * 2 + 1]);
