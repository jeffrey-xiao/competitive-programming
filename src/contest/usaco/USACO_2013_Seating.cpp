...
void pushUp(int n){
    ...
    if((tree[l(n)].seg == tree[n].seg && tree[l(n)].index < tree[n].index) || (tree[l(n)].seg > tree[n].seg)){
        tree[n].seg = tree[l(n)].seg;
        tree[n].index = tree[l(n)].index;
    }
    int posSeg = tree[l(n)].rightSeg + tree[r(n)].leftSeg;
    if(posSeg > tree[n].seg || (posSeg == tree[n].seg && tree[l(n)].right - tree[l(n)].rightSeg + 1 < tree[n].index)){
...
void pushDown(int n){
    ...
    if(tree[n].seg == 0){
        ...
        tree[l(n)].leftSeg = tree[l(n)].rightSeg = tree[l(n)].seg = tree[l(n)].right - tree[l(n)].left + 1;
        tree[r(n)].leftSeg = tree[r(n)].rightSeg = tree[r(n)].seg = tree[r(n)].right - tree[r(n)].left + 1;
