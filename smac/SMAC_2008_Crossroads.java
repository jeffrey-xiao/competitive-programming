...
public class SMAC_2008_Crossroads {
	...
	static BufferedReader buff = new BufferedReader(new InputStreamReader(
			System.in));
	...
	public static void main (String[] args) throws IOException {
		...
		for (int x = 1; x <= r; x++) {
			...
			for (int y = 1; y <= c; y++) {
				tl[x][y] = grid[x][y] + tl[x - 1][y] + tl[x][y - 1]
						- tl[x - 1][y - 1];
				total += grid[x][y];
			}
			// calculating right values
			for (int y = c; y >= 1; y--)
				tr[x][y] = grid[x][y] + tr[x - 1][y] + tr[x][y + 1]
						- tr[x - 1][y + 1];
		}
		...
		for (int x = r; x >= 1; x--) {
			// calculating left values
			for (int y = 1; y <= c; y++)
				bl[x][y] = grid[x][y] + bl[x + 1][y] + bl[x][y - 1]
						- bl[x + 1][y - 1];
			// calculating right values
			for (int y = c; y >= 1; y--)
				br[x][y] = grid[x][y] + br[x + 1][y] + br[x][y + 1]
						- br[x + 1][y + 1];
		}
		int max = 0;
		poss(2, 1);
	...
	private static boolean poss (int row, int col) {
		for (int x = 1; x + row - 1 <= r; x++) {
			for (int y = 1; y + col - 1 <= c; y++) {
				int cost = total - tl[x - 1][y - 1] - tr[x - 1][y + col]
						- bl[x + row][y - 1] - br[x + row][y + col];
				if (cost <= b)
					return true;
			}
		}
